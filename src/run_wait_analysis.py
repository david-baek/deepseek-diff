...
parser = argparse.ArgumentParser(description='Run wait analysis.')
parser.add_argument('--save_name', type=str, default="qwen-14b", help='Model name to save')
parser.add_argument('--target_name', type=str, default="wait", help='Target name')
...
for batch_idx, tokens in enumerate(all_tokens[:1000]):
    ...
    #wait_positions = [pos for i, token_id in enumerate(tokens_list) if token_id in wait_token_ids for pos in (i, i+1, i+2, i+3) if pos < 2048]
...
print(f"\n{target_name}: Found a total of {total_wait_tokens} target tokens in the processed sequences.\n")
...
import json
with open("../results/" + weight_path[3:-3].replace("/","__") + f"_{target_name}.json", 'w') as file:
